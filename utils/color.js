...
function longHexToNumber(H) {
  return parseInt(H, 16); // or +H
}

function hexToLongHex(H) {
  let r = 0;
  let g = 0;
  let b = 0;
...
export function getPageColors(post) {
  ...
  // const colorOptions = {
  //   custom: { background: post?.primaryColor?.hex, foreground: post?.secondaryColor?.hex },
  //   sanity: { background: palette?.background, foreground: palette?.foreground }
  // };
  // const colorHexes = paletteKey === "custom" ? colorOptions.custom : colorOptions.sanity;
  // const colorNumbers = {
  //   background: longHexToNumber(hexToLongHex(colorHexes.background)),
  //   foreground: longHexToNumber(hexToLongHex(colorHexes.foreground))
  // };

  // return getColors({
  const theColors = getColors({
  ...
  const baseBg = longHexToNumber(hexToLongHex(theColors.base.background.hex));
  const baseFg = longHexToNumber(hexToLongHex(theColors.base.foreground.hex));
  const compBg = longHexToNumber(hexToLongHex(theColors.comp.background.hex));
  const compFg = longHexToNumber(hexToLongHex(theColors.comp.foreground.hex));

  console.log("[Test]", {
    base: {
      background: theColors.base.background.hex,
      foreground: theColors.base.foreground.hex,
      contrast: APCAcontrast(sRGBtoY(baseFg), sRGBtoY(baseBg))
    },
    comp: {
      background: theColors.comp.background.hex,
      foreground: theColors.comp.foreground.hex,
      contrast: APCAcontrast(sRGBtoY(compFg), sRGBtoY(compBg))
    }
  });
...
/***********************************************************************/
/* COPIED FROM `node_modules/apca-w3/dist` BECUASE IT EXPORTS NOTHING! */
/***********************************************************************/
