...
export async function getFeaturedNovel(preview) {
  return await getClient(preview).fetch(
    `*[_type == "homePage"][0].featuredNovel->{title, slug, overview, images}`
  );
}

export async function getFeaturedReviews(preview) {
  return await getClient(preview).fetch(
    `*[_type == "homePage"][0].featuredReviews[]->{review, reviewer, title}`
  );
}

export async function getAuthorBio(preview) {
  return await getClient(preview).fetch(`*[_type == "author"][0]{name, image, biography}`);
}

export async function getRecentPosts(preview, limit) {
  return await getClient(preview).fetch(
    `*[_type == "post"][0..${limit - 1}] | order(publishedAt desc){title, slug, mainImage}`
  );
}
